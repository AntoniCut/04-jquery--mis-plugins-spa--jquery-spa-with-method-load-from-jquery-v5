...
export const PAGE_ESM_FEATURES = Object.freeze([
    {
        ...
        "text": "Lista de widgets renderizada con ES Modules nativos."
    },
    {
        "icon": "🔄",
        "title": "Fase 3 del plugin",
        "text": "applyRouteMetaAsync ejecuta scripts tras View Transition."
    },
    {
        "icon": "📡",
        "title": "Evento spa:route-loaded",
        "text": "Confirma ruta jqueryUiPage al terminar la carga."
    }
]);
...
export const createFeatureArticle = ({ icon, title, text }) => {
    ...
    article.className = 'jquery-ui-page__feature';

    article.innerHTML = `
        <span class="jquery-ui-page__feature-icon" aria-hidden="true">${icon}</span>
        <h4 class="jquery-ui-page__feature-title">${title}</h4>
        <p class="jquery-ui-page__feature-text">${text}</p>
    `;
...
export const createTagItem = (label) => {
    ...
    item.className = 'jquery-ui-page__tag';
...
const createRouteLoadedCard = () => {
    ...
    output.className = 'jquery-ui-page__feature-text';
    ...
    article.innerHTML = `
        <span class="jquery-ui-page__feature-icon" aria-hidden="true">📡</span>
        <h4 class="jquery-ui-page__feature-title">Integración SPA</h4>
    `;
 ...
 * -----  `renderPageDemoEsm()`  -----
 * ------------------------------------
 * - Renderiza las tarjetas ESM en jquery-ui-demo.html.
 */
export const renderPageDemoEsm = () => {

    /** - Contenedor de las tarjetas ESM */
    const target = document.querySelector('[data-jquery-ui-demo-target="esm"]');

    /** - Elemento de estado */
    const status = document.querySelector('[data-jquery-ui-demo-status="esm"]');

    if (!target) {
        console.warn('⚠️ jquery-ui-page.esm.js: contenedor [data-jquery-ui-demo-target="esm"] no encontrado.');
        return;
    }
    ...
    setDemoStatus(
        status,
        `Renderizado con jquery-ui-page.esm.js (ESM) · ${PLUGIN_NAME} v${PLUGIN_VERSION} · v${VERSION} · ${new Date().toLocaleTimeString()}`
    );

    console.warn('-----  jquery-ui-page.esm.js  -----  ES Module  -----');
...
export const renderPageTags = () => {
    ...
    const target = document.querySelector('[data-jquery-ui-demo-target="tags"]');

    if (!target) {
        console.warn('⚠️ jquery-ui-page.esm.js: contenedor [data-jquery-ui-demo-target="tags"] no encontrado.');
        return;
    }
